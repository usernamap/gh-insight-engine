...
AuthController.login = (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  try {
    const tokenValidation =
      await github_1.githubConfig.validateToken(githubToken);
    ...
    const userProfile = await github_1.githubConfig
      .getGitHubService()
      .getUserProfile(username, githubToken);
    if (!userProfile) {
    ...
    const savedUser = await DatabaseService_1.databaseService.saveUserDataset({
      ...
      metadata: {
        ...
        totalRepositories:
          userProfile.publicRepos + (userProfile.privateRepos || 0),
...
AuthController.refresh = (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  try {
    const tokenValidation = await github_1.githubConfig.validateToken(
      user.githubToken,
    );
    if (!tokenValidation.valid) {
      logger_1.logWithContext.auth(
...
AuthController.validateToken = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const user = req.user;
    if (!user) {
...
AuthController.getCurrentUser = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const user = req.user;
    if (!user) {
      throw errorHandler_2.createError.authentication("Token JWT requis");
    ...
    try {
      const userData = await DatabaseService_1.databaseService.getUser(
        user.username,
      );
      if (!userData) {
