...
UserController.getUserAnalysisStatus = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const latestDataset = await DatabaseService_1.databaseService.getLatestDatasetForUser(userData.id);
        const analysisAge = await DatabaseService_1.databaseService.checkAnalysisFreshness(userData.id);
        const responseData = {
            user: {
                id: userData.id,
                username: userData.login,
...
UserController.deleteUserData = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        const deletionResult = await DatabaseService_1.databaseService.deleteUserData(username);
        logger_1.logWithContext.api('delete_user_data', req.path, true, {
            targetUsername: username,
