...
AnalyticsController.analyzeUser = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    ...
    if (!authenticatedUser?.githubToken) {
      throw errorHandler_2.createError.authentication(
        "Token GitHub requis pour l'analyse",
      );
    }
    ...
    try {
      ...
      const savedDataset =
        await DatabaseService_1.databaseService.saveUserDataset({
          userProfile,
          repositories,
          metadata: {
            ...
            scope: analysisParams.includePrivate
              ? "user_repositories"
              : "public_repositories",
      ...
      logger_1.logWithContext.analytics(
        "generate_analytics_start",
        username,
        true,
      );
      const analyticsOverview =
        await AnalyticsService_1.analyticsService.generateAnalyticsOverview(
          userProfile,
          repositories,
        );
      await DatabaseService_1.databaseService.updateDatasetAnalytics(
        savedDataset.id,
        analyticsOverview,
      );
      logger_1.logWithContext.analytics(
        "generate_analytics_complete",
        username,
        true,
        {
        ...
        analytics: {
          ...
          productivity: {
            ...
            averageCommitsPerRepo:
              analyticsOverview.productivity.averageCommitsPerRepo,
          },
          languages: {
...
AnalyticsController.getAnalyticsOverview = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      res.status(200).json({
        ...
        metadata: {
          ...
          repositoriesAnalyzed: Array.isArray(latestDataset.repositories)
            ? latestDataset.repositories.length
            : 0,
...
AnalyticsController.getPerformanceMetrics = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.analytics) {
        throw errorHandler_2.createError.notFound(
          "Aucune analyse de performance trouvée",
        );
      }
      const analytics = latestDataset.analytics;
...
AnalyticsController.getLanguageAnalytics = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.analytics) {
        throw errorHandler_2.createError.notFound(
          "Aucune analyse de langages trouvée",
        );
      }
      const analytics = latestDataset.analytics;
      ...
      res.status(200).json({
        ...
        languages: {
          ...
          diversity: {
            ...
            description:
              languages.diversityIndex > 0.7
                ? "Très diversifié"
                : languages.diversityIndex > 0.5
                  ? "Diversifié"
                  : languages.diversityIndex > 0.3
          ...
          experience: {
            senior: languages.languages.filter(
              (l) => l.experienceLevel === "Senior",
            ).length,
            intermediate: languages.languages.filter(
              (l) => l.experienceLevel === "Intermediate",
            ).length,
            beginner: languages.languages.filter(
              (l) => l.experienceLevel === "Beginner",
            ).length,
          },
...
AnalyticsController.getActivityPatterns = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.analytics) {
        throw errorHandler_2.createError.notFound(
          "Aucune analyse d'activité trouvée",
        );
      }
      const analytics = latestDataset.analytics;
...
AnalyticsController.getProductivityScore = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.analytics) {
        throw errorHandler_2.createError.notFound(
          "Aucune analyse de productivité trouvée",
        );
      }
      const analytics = latestDataset.analytics;
      ...
      res.status(200).json({
        ...
        productivity: {
          overall: {
            ...
            level:
              productivity.overallScore >= 80
                ? "Très élevée"
                : productivity.overallScore >= 60
                  ? "Élevée"
                  : productivity.overallScore >= 40
                    ? "Modérée"
                    : "Faible",
          },
          commits: {
...
AnalyticsController.getDevOpsMaturity = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
