...
RepoController.getRepository = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        const repositoryData = await DatabaseService_1.databaseService.getRepository(nameWithOwner);
        if (!repositoryData) {
            if (authenticatedUser?.githubToken) {
                try {
                    logger_1.logWithContext.github('fetch_repository_from_api', nameWithOwner, true);
...
RepoController.enrichRepository = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        const existingRepo = await DatabaseService_1.databaseService.getRepository(nameWithOwner);
        if (!existingRepo) {
            throw errorHandler_2.createError.notFound('Repository non trouvé en base de données');
        }
        const enrichedData = await GitHubService_1.githubService.enrichWithDevOpsData(owner, repo, authenticatedUser.githubToken);
        if (!enrichedData) {
            throw errorHandler_2.createError.externalService('GitHub API', new Error('Impossible d\'enrichir le repository'));
        }
        const updatedRepo = await DatabaseService_1.databaseService.enrichRepository(nameWithOwner, {
...
RepoController.getTrendingRepositories = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const trendingRepos = await DatabaseService_1.databaseService.searchRepositories({
        ...
        const recentRepos = await DatabaseService_1.databaseService.searchRepositories({
