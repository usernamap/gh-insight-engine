...
InsightsController.generateInsights = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    ...
    try {
      ...
      const repositories =
        await DatabaseService_1.databaseService.getUserRepositories(
          userData.id,
        );
      logger_1.logWithContext.ai("ai_analysis_start", username, true);
      const aiInsights = await AIService_1.aiService.generateInsights(
        userData,
        repositories,
        latestDataset.analytics,
      );
      await DatabaseService_1.databaseService.updateDatasetInsights(
        latestDataset.id,
        aiInsights,
      );
...
InsightsController.getInsightsSummary = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
...
InsightsController.getDeveloperPersonality = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.aiInsights) {
        throw errorHandler_2.createError.notFound(
          "Aucune analyse de personnalité trouvée",
        );
      }
      const aiInsights = latestDataset.aiInsights;
...
InsightsController.getRecommendations = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    ...
    try {
      ...
      if (
        category &&
        ["immediate", "shortTerm", "longTerm"].includes(category)
      ) {
      ...
      res.status(200).json({
        ...
        metadata: {
          generatedAt: latestDataset.updatedAt,
          basedOnAnalysis:
            "AI-powered analysis of coding patterns and project history",
        },
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
...
InsightsController.getStrengths = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.aiInsights) {
        throw errorHandler_2.createError.notFound(
          "Aucune analyse des forces trouvée",
        );
      }
      const aiInsights = latestDataset.aiInsights;
...
InsightsController.getGrowthOpportunities = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      res.status(200).json({
        ...
        growth: {
          ...
          priority: {
            high:
              growth.areas?.filter((area) => area.priority === "high") || [],
            medium:
              growth.areas?.filter((area) => area.priority === "medium") || [],
...
InsightsController.getSkillAssessment = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
...
InsightsController.getCareerInsights = (0, errorHandler_1.asyncHandler)(
  async (req, res) => {
    const { username } = req.params;
    const authenticatedUser = req.user;
    ...
    try {
      ...
      if (!latestDataset || !latestDataset.aiInsights) {
        throw errorHandler_2.createError.notFound(
          "Aucun insight de carrière trouvé",
        );
      }
      const aiInsights = latestDataset.aiInsights;
