import { PrismaClient } from '@prisma/client';
import mongoose from 'mongoose';
export declare class DatabaseConfig {
    private prismaClient;
    private mongooseConnection;
    private isConnected;
    initialize(): Promise<void>;
    private initializePrisma;
    private initializeMongoose;
    healthCheck(): Promise<{
        prisma: boolean;
        mongoose: boolean;
        overall: boolean;
    }>;
    transaction<T>(operations: (prisma: PrismaClient) => Promise<T>): Promise<T>;
    createIndexes(): Promise<void>;
    cleanup(): Promise<void>;
    cleanupTestData(): Promise<void>;
    private sanitizeUrl;
    getPrismaClient(): PrismaClient | null;
    getMongooseConnection(): mongoose.Connection | null;
    getConnectionStatus(): boolean;
    findMany<T>(model: string, options?: Record<string, unknown>): Promise<T[]>;
    findUnique<T>(model: string, where: Record<string, unknown>): Promise<T | null>;
    create<T>(model: string, data: Record<string, unknown>): Promise<T>;
    update<T>(model: string, where: Record<string, unknown>, data: Record<string, unknown>): Promise<T>;
    delete<T>(model: string, where: Record<string, unknown>): Promise<T>;
}
export declare const databaseConfig: DatabaseConfig;
export default databaseConfig;
//# sourceMappingURL=database.d.ts.map