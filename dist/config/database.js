...
class DatabaseConfig {
    ...
    async initialize() {
        ...
        try {
            ...
            logger_1.default.info('Configuration base de données initialisée avec succès', {
                providers: ['Prisma', 'Mongoose'],
                url: this.sanitizeUrl(databaseUrl),
            });
        }
        catch (error) {
            logger_1.default.error('Échec de l\'initialisation de la base de données', {
                error: error.message,
                stack: error.stack,
            });
    ...
    async transaction(operations) {
        ...
        try {
            ...
            logger_1.default.debug('Transaction Prisma terminée', { duration: `${duration}ms` });
            return result;
        }
        catch (error) {
            const duration = Date.now() - startTime;
    ...
    async createIndexes() {
        ...
        try {
            ...
            logger_1.default.error('Erreur lors de la création des indexes', { error: error.message });
            throw error;
        }
    }
    async cleanup() {
        try {
            ...
            logger_1.default.error('Erreur lors de la fermeture des connexions', { error: error.message });
        }
    }
    async cleanupTestData() {
        ...
        try {
            ...
            logger_1.default.error('Erreur lors du nettoyage des données de test', { error: error.message });
            throw error;
        }
    }
    sanitizeUrl(url) {
