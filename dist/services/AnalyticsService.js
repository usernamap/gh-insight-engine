...
class AnalyticsService {
  ...
  async analyzeDevOpsMaturity(repositories) {
    try {
      ...
      const reposWithCI = repositories.filter(
        (repo) => repo.githubActions?.workflowsCount > 0,
      ).length;
  ...
  calculateCodeReviewParticipation(repositories) {
    const reposWithPRs = repositories.filter(
      (repo) => repo.pullRequests.totalCount > 0,
    ).length;
  ...
  calculateAverageIssueResolutionTime(repositories) {
    const totalIssues = repositories.reduce(
      (sum, repo) => sum + repo.issues.totalCount,
      0,
    );
    const openIssues = repositories.reduce(
      (sum, repo) => sum + repo.issues.openCount,
      0,
    );
  ...
  calculateLeadershipScore(userProfile, repositories) {
    ...
    const teamRepos = repositories.filter(
      (repo) => repo.collaborators.totalCount > 1,
    ).length;
    return Math.min(
      100,
