...
class AnalyticsService {
    async generateAnalyticsOverview(_userProfile, repositories, _timeframe) {
        ...
        try {
            ...
            const [performance, productivity, languages, activity, complexity, devops, collaboration,] = await Promise.all([
                this.calculatePerformanceMetrics(_userProfile, repositories, _timeframe),
            ...
            const timeframeObj = _timeframe
                ? {
                    ..._timeframe,
                    totalDays: Math.ceil((_timeframe.end.getTime() - _timeframe.start.getTime()) /
                        (1000 * 60 * 60 * 24)),
                }
                : {
            ...
            throw new Error(`Génération analytics échouée: ${_error.message}`);
        }
    }
    async calculatePerformanceMetrics(_userProfile, repositories, _timeframe) {
        try {
            ...
            const relevantCommits = _timeframe
                ? allCommits.filter((commit) => {
                    ...
                    return (commitDate >= _timeframe.start && commitDate <= _timeframe.end);
                })
                : allCommits;
            ...
            const averageCommitSize = relevantCommits.length > 0
                ? relevantCommits.reduce((sum, commit) => sum + (commit.additions ?? 0) + (commit.deletions ?? 0), 0) / relevantCommits.length
                : 0;
            ...
            logger_1.default.error('Erreur calcul métriques performance', {
                _error: _error.message,
            });
            throw new Error(`Erreur calcul métriques performance: ${_error.message}`);
        }
    }
    async calculateProductivityScore(_userProfile, repositories) {
        try {
            ...
            const activeRepos = repositories.filter((repo) => repo.isArchived === false).length;
            ...
            const overall = Math.round(consistency * 0.25 + volume * 0.25 + impact * 0.25 + maintenance * 0.25);
            ...
            logger_1.default.error('Erreur calcul score productivité', {
                _error: _error.message,
            });
            throw new Error(`Erreur calcul score productivité: ${_error.message}`);
        }
    }
    async analyzeLanguages(repositories) {
        try {
            ...
            logger_1.default.error('Erreur analyse langages', {
                _error: _error.message,
            });
            throw new Error(`Erreur analyse langages: ${_error.message}`);
        }
    }
    async analyzeActivityPatterns(repositories) {
        try {
            const allCommits = repositories.flatMap((repo) => repo.commits.recent);
            ...
            logger_1.default.error("Erreur analyse patterns d'activité", {
                _error: _error.message,
            });
            throw new Error(`Erreur analyse patterns d'activité: ${_error.message}`);
        }
    }
    async analyzeProjectComplexity(repositories) {
        try {
            ...
            repositories.forEach((repo) => {
                ...
                const hasSecurity = (repo.security?.dependabotAlerts?.totalCount ?? 0) > 0 ||
            ...
            const averageComplexity = total > 0
                ? Math.round((simple * 25 + moderate * 50 + complex * 75 + enterprise * 100) /
                    total)
                : 0;
            return {
                simple,
            ...
            logger_1.default.error('Erreur analyse complexité projets', {
                _error: _error.message,
            });
            throw new Error(`Erreur analyse complexité projets: ${_error.message}`);
        }
    }
    async analyzeDevOpsMaturity(repositories) {
        try {
            ...
            const reposWithCI = repositories.filter((repo) => (repo.githubActions?.workflowsCount ?? 0) > 0).length;
            ...
            const reposWithTests = repositories.filter((repo) => (repo.githubActions?.workflows?.some((workflow) => workflow.name.toLowerCase().includes('test') ||
                workflow.name.toLowerCase().includes('ci'))) ?? false).length;
            const testingCulture = Math.round((reposWithTests / totalRepos) * 100);
            const reposWithSecurity = repositories.filter((repo) => (repo.security?.dependabotAlerts?.totalCount ?? 0) > 0 ||
                (repo.security?.hasSecurityPolicy ?? false) ||
                (repo.branchProtection?.rules?.length ?? 0) > 0).length;
            const securityPractices = Math.round((reposWithSecurity / totalRepos) * 100);
            const reposWithDocs = repositories.filter((repo) => (repo.community?.hasReadme ?? repo.community?.hasContributing) === true ||
            ...
            logger_1.default.error('Erreur analyse maturité DevOps', {
                _error: _error.message,
            });
            throw new Error(`Erreur analyse maturité DevOps: ${_error.message}`);
        }
    }
    async analyzeCollaborationMetrics(_userProfile, repositories) {
        try {
            const teamProjects = repositories.filter((repo) => repo.forkCount > 0 ||
                repo.stargazerCount > 5 ||
                (repo.collaborators.totalCount ?? 0) > 1).length;
            const soloProjects = repositories.length - teamProjects;
            const contributionsToOthers = repositories.filter((repo) => repo.isFork && (repo.commits.totalCount ?? 0) > 0).length;
            const pullRequestsReceived = repositories.reduce((sum, repo) => sum + (repo.pullRequests.totalCount ?? 0), 0);
            const pullRequestsMade = contributionsToOthers * 2;
            const codeReviewsGiven = Math.round(pullRequestsReceived * 0.3);
            const mentorshipActivity = this.calculateMentorshipScore(_userProfile, repositories);
            const leadershipScore = this.calculateLeadershipScore(_userProfile, repositories);
            return {
                teamProjects,
                soloProjects,
            ...
            logger_1.default.error('Erreur analyse métriques collaboration', {
                _error: _error.message,
            });
            throw new Error(`Erreur analyse métriques collaboration: ${_error.message}`);
        }
    }
    ...
    detectBranchingStrategy(repositories) {
        ...
        const hasDevelop = repositories.some((repo) => (repo.branchProtection?.rules?.some((rule) => rule.pattern.includes('develop'))) ?? false);
        if (hasDevelop)
            return 'gitflow';
    ...
    calculatePullRequestRatio(repositories, commits) {
        const totalPRs = repositories.reduce((sum, repo) => sum + (repo.pullRequests.totalCount ?? 0), 0);
    ...
    calculateCodeReviewParticipation(repositories) {
        const reposWithPRs = repositories.filter((repo) => (repo.pullRequests.totalCount ?? 0) > 0).length;
        const reposWithReviewRules = repositories.filter((repo) => (repo.branchProtection?.rules?.some((rule) => rule.requiresCodeOwnerReviews)) ?? false).length;
        const total = repositories.length;
        if (total === 0)
            return 0;
    ...
    calculateAverageIssueResolutionTime(repositories) {
        const totalIssues = repositories.reduce((sum, repo) => sum + (repo.issues.totalCount ?? 0), 0);
        const openIssues = repositories.reduce((sum, repo) => sum + (repo.issues.openCount ?? 0), 0);
        if (totalIssues === 0)
            return 0;
    ...
    analyzeSeasonality(commits) {
        ...
        const variance = quarterCounts.reduce((sum, count) => sum + Math.pow(count - avg, 2), 0) /
            4;
    ...
    calculateMentorshipScore(_userProfile, repositories) {
        const publicRepos = repositories.filter((repo) => !repo.isPrivate).length;
        const docsRepos = repositories.filter((repo) => (repo.community?.hasReadme ?? repo.community?.hasContributing) === true).length;
    ...
    calculateLeadershipScore(_userProfile, repositories) {
        ...
        const teamRepos = repositories.filter((repo) => (repo.collaborators.totalCount ?? 0) > 1).length;
