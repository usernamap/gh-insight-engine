...
class DatabaseService {
    ...
    try {
      ...
      return await database_1.default.transaction(async (prisma) => {
        ...
        for (const repo of repositories) {
          const savedRepo = await models_1.RepositoryModel.upsert(
            repo,
            user.id,
          );
        ...
        let dataset = await models_1.DatasetModel.create(
          user.id,
          metadata,
          repositoryIds,
        );
        if (analytics) {
          dataset = await models_1.DatasetModel.updateAnalytics(
            dataset.id,
            analytics,
          );
        }
        if (insights) {
          dataset = await models_1.DatasetModel.updateInsights(
            dataset.id,
            insights,
          );
        }
  ...
  async getLatestUserDataset(username) {
    try {
      ...
      logger_1.default.error(
        "Erreur récupération dernier dataset utilisateur",
        {
          username,
          error: error.message,
        },
  ...
  async enrichRepositoriesWithDevOpsData(repositoryIds, devOpsDataMap) {
    try {
      ...
      for (const repositoryId of repositoryIds) {
        ...
        if (devOpsData) {
          const enrichedRepo =
            await models_1.RepositoryModel.enrichWithDevOpsData(
              repositoryId,
              devOpsData,
            );
  ...
  async searchRepositoriesWithUserInfo(filters) {
    try {
      const { repositories, total } =
        await models_1.RepositoryModel.search(filters);
      ...
      logger_1.default.error(
        "Erreur recherche repositories avec info utilisateur",
        {
          filters,
          error: error.message,
        },
  ...
  async areUserAnalyticsUpToDate(username, maxAgeMinutes = 60) {
    try {
      ...
      const freshness = await models_1.DatasetModel.isAnalyticsUpToDate(
  ...
  async deleteAllUserData(username) {
    try {
      logger_1.default.info("Début suppression données utilisateur", {
        username,
      });
      const user = await models_1.UserModel.findByLogin(username);
      if (!user) {
  ...
  async cleanupOldData(options = {}) {
    ...
    try {
      ...
      if (!dryRun) {
        datasetsDeleted =
          await models_1.DatasetModel.cleanupOldDatasets(olderThanDays);
      }
  ...
  async findInactiveUsers(olderThanDays) {
    try {
      ...
      const inactive = await database_1.default
        .getPrismaClient()
        ?.user.findMany({
          where: {
