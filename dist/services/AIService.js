...
class AIService {
  ...
  async analyzeDeveloperPersonality(userProfile, repositories, analytics) {
    try {
      const analysisResult = await openai_1.default.generateDeveloperSummary(
        userProfile,
        repositories,
        analytics,
      );
      ...
      return {
        ...
        strengths: Array.isArray(result.strengths)
          ...
          : [],
        workingStyle,
        motivations: Array.isArray(result.motivations)
          ? result.motivations.slice(0, 4)
          : [],
        potentialChallenges: Array.isArray(result.potentialChallenges)
          ? result.potentialChallenges.slice(0, 3)
          : [],
      };
    } catch (error) {
  ...
  async assessTechnicalSkills(userProfile, repositories, analytics) {
    try {
      const analysisResult = await openai_1.default.assessTechnicalSkills(
        userProfile,
        repositories,
        analytics,
      );
  ...
  async analyzeCareerInsights(userProfile, repositories, analytics) {
    try {
      const analysisResult = await openai_1.default.analyzeCareerInsights(
        userProfile,
        repositories,
        analytics,
      );
      ...
      return {
        ...
        trajectory: result.trajectory
          ? this.validateTrajectory(result.trajectory)
          : {
              direction: "stable",
              velocity: "steady",
              confidence: 70,
      ...
      const patterns = {
        ...
        consistency: {
          level: this.mapConsistencyLevel(
            analytics.productivity.breakdown.consistency,
          ),
  ...
  async generateTechnicalRecommendations(userProfile, repositories, analytics) {
    try {
      const analysisResult = await openai_1.default.generateRecommendations(
        userProfile,
        repositories,
        analytics,
      );
  ...
  validateWorkingStyle(style) {
    return {
      preferredProjectSize: ["small", "medium", "large", "mixed"].includes(
        style?.preferredProjectSize,
      )
        ? style.preferredProjectSize
        : "medium",
      collaborationStyle: ["solo", "pair", "team_lead", "contributor"].includes(
      ...
      learningApproach: [
        ...
        "research_driven",
        "hands_on",
      ].includes(style?.learningApproach)
        ? style.learningApproach
        : "hands_on",
      problemSolving: [
        "analytical",
        "creative",
        "systematic",
        "intuitive",
      ].includes(style?.problemSolving)
        ? style.problemSolving
        : "systematic",
    };
  }
  ...
  generateFallbackPersonality(userProfile, repositories, analytics) {
    ...
    return {
      ...
      workingStyle: {
        ...
        collaborationStyle:
          analytics.collaboration.teamProjects >
          analytics.collaboration.soloProjects
            ? "contributor"
            : "solo",
  ...
  inferArchetypeFromData(repositories, analytics) {
    const forkRatio =
      repositories.filter((r) => r.isFork).length / repositories.length;
  ...
  generateFallbackTechnicalSkills(repositories) {
    ...
    return Object.entries(languageStats)
      ...
      .map(([language, stats]) => ({
        ...
        evidenceStrength:
          stats.count >= 5 ? "strong" : stats.count >= 3 ? "moderate" : "weak",
  ...
  generateFallbackLeadershipSkill(userProfile, repositories) {
    ...
    return {
      ...
      potential,
      indicators: [
        `${ownedRepos} repositories propres`,
  ...
  generateWorkLifeRecommendations(analytics) {
    ...
    if (
      analytics.collaboration.soloProjects >
      analytics.collaboration.teamProjects * 3
    ) {
  ...
  identifyCoreStrengths(analytics) {
    ...
    if (analytics.languages.distribution.length > 4) {
      strengths.push({
        ...
        evidence: [
          `${analytics.languages.distribution.length} langages maîtrisés`,
        ],
      ...
      core: [
        {
          ...
          evidence: [
            `Score consistance: ${analytics.productivity.breakdown.consistency}/100`,
          ],
  ...
  validateTechnicalSkill(skill) {
    return {
      ...
      evidenceStrength: ["weak", "moderate", "strong", "very_strong"].includes(
      ...
      evidence: Array.isArray(skill.evidence)
        ...
        : ["Usage détecté"],
      growthPotential: ["limited", "moderate", "high", "exceptional"].includes(
        skill.growthPotential,
      )
        ? skill.growthPotential
        : "moderate",
      marketDemand: ["low", "moderate", "high", "very_high"].includes(
        skill.marketDemand,
      )
        ? skill.marketDemand
        : "moderate",
  ...
  validateSoftSkill(skill) {
    return {
      skill: skill.skill || "Unknown",
      level: ["developing", "competent", "strong", "exceptional"].includes(
        skill.level,
      )
        ? skill.level
        : "competent",
      indicators: Array.isArray(skill.indicators)
        ? skill.indicators.slice(0, 3)
        : [],
      impactOnCareer: ["minor", "moderate", "significant", "critical"].includes(
  ...
  validateLeadershipSkill(leadership) {
    return {
      current: [
        ...
        "team_lead",
        "senior_leader",
      ].includes(leadership.current)
        ? leadership.current
        : "individual_contributor",
      potential: ["limited", "emerging", "strong", "exceptional"].includes(
        leadership.potential,
      )
        ? leadership.potential
        : "emerging",
      indicators: Array.isArray(leadership.indicators)
        ? leadership.indicators.slice(0, 3)
        : [],
    };
  }
  validateTrajectory(trajectory) {
    return {
      direction: ["ascending", "stable", "transitioning", "exploring"].includes(
        trajectory.direction,
      )
        ? trajectory.direction
        : "stable",
      velocity: ["slow", "steady", "rapid", "exponential"].includes(
        trajectory.velocity,
      )
        ? trajectory.velocity
        : "steady",
  ...
  validateMarketPosition(position) {
    return {
      competitiveness: [
        ...
        "above_average",
        "exceptional",
      ].includes(position.competitiveness)
        ? position.competitiveness
        : "average",
      uniqueValueProposition:
        position.uniqueValueProposition || "Profil technique solide",
      differentiators: Array.isArray(position.differentiators)
        ? position.differentiators.slice(0, 4)
        : [],
  ...
  validateRecommendation(rec) {
    return {
      category: ["skill", "tool", "practice", "project"].includes(rec.category)
        ? rec.category
        : "skill",
      ...
      expectedImpact: [
        ...
        "significant",
        "transformative",
      ].includes(rec.expectedImpact)
        ? rec.expectedImpact
        : "moderate",
      effort: ["low", "medium", "high"].includes(rec.effort)
        ? rec.effort
        : "medium",
  ...
  validateLongTermVision(vision) {
    return {
      ...
      milestones: Array.isArray(vision.milestones)
        ? vision.milestones.slice(0, 4)
        : [],
      skills: Array.isArray(vision.skills) ? vision.skills.slice(0, 4) : [],
      experience: Array.isArray(vision.experience)
        ? vision.experience.slice(0, 4)
        : [],
    };
  }
}
