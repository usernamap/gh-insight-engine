...
class GitHubService {
    ...
    `;
        try {
            ...
            logger_1.default.error('Erreur récupération organisations', { error: error.message });
    ...
    `;
        try {
            ...
            logger_1.default.error('Erreur récupération profil utilisateur', { error: error.message });
    ...
    `;
        try {
            ...
            const repos = repositories.map((repo) => {
                const languageNodes = repo.languages?.edges?.map((edge) => ({
                    name: edge.node.name,
                    size: edge.size,
                ...
                const commits = {
                    ...
                    recent: commitHistory?.nodes?.map((commit) => ({
                        oid: commit.oid,
            ...
            logger_1.default.error('Erreur récupération repositories utilisateur', { error: error.message });
        ...
        try {
            ...
            const securityData = {
                dependabotAlerts: {
                    ...
                    open: dependabotAlerts.filter((alert) => alert.state === 'open').length || 0,
                    fixed: dependabotAlerts.filter((alert) => alert.state === 'fixed').length || 0,
                    dismissed: dependabotAlerts.filter((alert) => alert.state === 'dismissed').length || 0,
                },
                secretScanning: {
                    totalCount: secretAlerts.length || 0,
                    resolved: secretAlerts.filter((alert) => alert.state === 'resolved').length || 0,
                },
                codeScanning: {
                    totalCount: codeAlerts.length || 0,
                    open: codeAlerts.filter((alert) => alert.state === 'open').length || 0,
                    fixed: codeAlerts.filter((alert) => alert.state === 'fixed').length || 0,
                },
    ...
    async getPackagesData(owner, repo) {
        try {
            ...
            const packageTypes = [...new Set(repoPackages.map((pkg) => pkg.package_type))];
            const packagesData = {
