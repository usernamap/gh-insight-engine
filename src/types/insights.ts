/**
 * Types pour les analyses IA et insights qualitatifs
...
export interface ProductivityAnalysis {
  ...
  workLifeBalance: {
    sustainabilityScore: number; // Score 0-100
    riskFactors: string[];
...
export interface InsightsResponse {
  ...
  error?: string;
  partial?: boolean; // Si seulement certaines analyses ont réussi
  cacheInfo?: {
    cached: boolean;
    generatedAt: Date;
    expiresAt: Date;
...
// Types pour la validation et le feedback
export interface InsightValidation {
  insightId: string;
  userId: string;
  accuracy: number; // Score 1-5
  usefulness: number; // Score 1-5
  feedback: string;
  categories: string[];
  timestamp: Date;
}
