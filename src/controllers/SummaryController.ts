...
/**
 * Contrôleur Summary - Analytics ultra-complets pour intégrations
 * Optimisé pour portfolios, CV et présentations professionnelles
 * Basé sur les VRAIES données GitHub collectées et stockées
 */
export class SummaryController {
  ...
  static getDeveloperSummary = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        // Vérifier les données utilisateur disponibles
        const userData = await UserModel.findByLogin(username);
        if (userData == null) {
          throw createError.notFound(
            'Aucune donnée trouvée pour cet utilisateur. Utilisez POST /users/{username} pour collecter les données.',
          );
        }

        // Récupérer TOUTES les données repositories réelles
        ...
        // 1. Métriques de base (vraies données)
        ...
        const totalWatchers = repositories.reduce(
          (sum, repo) => sum + repo.watchersCount,
          0,
        );
        ...
        // NOUVELLES MÉTRIQUES POUR L'INTERFACE

        // 1. Calcul lignes de code totales
        ...
        // 2. Classification par organisation avec typage correct
          ...
          if (repo.languages != null && typeof repo.languages === 'object') {
            ...
            if (languages.nodes != null) {
              languages.nodes.forEach((lang) => {
                ...
                if (
                  !languageStats[lang.name].repositories.includes(repo.name)
                ) {
        ...
        // 3. Tri des langages par utilisation
        const sortedLanguages = Object.entries(languageStats)
          .sort(([, a], [, b]) => b.totalBytes - a.totalBytes)
          .slice(0, 10);

        const primaryLanguages = sortedLanguages
          .slice(0, 5)
          .map(([name, stats]) => ({
            name,
            proficiencyLevel: Math.min(
              100,
              Math.round(stats.totalBytes / 10000 + stats.count * 5),
            ),
        ...
        const emergingLanguages = sortedLanguages
          ...
          .map(([name, stats]) => ({
            ...
            proficiencyLevel: Math.min(
              100,
              Math.round(stats.totalBytes / 20000 + stats.count * 3),
            ),
            totalBytes: stats.totalBytes,
            repositoryCount: stats.count,
            recentUsage: true,
        ...
        // 3. Calcul de l'activité récente (basée sur pushedAt)
        const now = new Date();
        ...
        const recentRepos30 = repositories.filter(
          (repo) =>
            repo.pushedAt != null && new Date(repo.pushedAt) > last30Days,
        ).length;
        const recentRepos90 = repositories.filter(
          (repo) =>
            repo.pushedAt != null && new Date(repo.pushedAt) > last90Days,
        ).length;
        const recentReposYear = repositories.filter(
          (repo) => repo.pushedAt != null && new Date(repo.pushedAt) > lastYear,
        ).length;

        // 4. Calculs de scores basés sur vraies données
        ...
        // 7. Calcul des niveaux de technologies
        const technologyLevels = {
          expert: primaryLanguages.filter((lang) => lang.proficiencyLevel >= 80)
            .length,
        ...
        // GÉNÉRATION DU SUMMARY AVEC VRAIES DONNÉES
        const summary = {
          profile: {
            ...
            specializations: primaryLanguages.slice(0, 2).map((lang) => ({
              ...
              yearsOfExperience: Math.min(
                activeYears,
                Math.round(lang.repositoryCount / 3),
              ),
          ...
          portfolioOverview: {
            ...
            publicRepositories,
            privateRepositories,
            contributedRepositories: repositories.filter((r) => r.isFork)
              .length,
            ...
            // Classification des projets basée sur les vraies données
            projectTypes: {
              ...
              libraries: repositories.filter(
                (r) =>
                  r.topics.includes('library') || r.topics.includes('package'),
              ).length,
              frameworks: repositories.filter((r) =>
                r.topics.includes('framework'),
              ).length,
              tools: repositories.filter(
                (r) => r.topics.includes('tool') || r.topics.includes('cli'),
              ).length,
              ...
              iot: repositories.filter(
                (r) => r.topics.includes('iot') || r.topics.includes('arduino'),
              ).length,
              blockchain: repositories.filter((r) =>
                r.topics.some((topic) =>
                  ['blockchain', 'crypto', 'web3'].includes(topic),
                ),
              ).length,
              other: repositories.filter(
                (r) => r.topics.length === 0 && r.primaryLanguage == null,
              ).length,
            },

            projectMaturity: {
              prototype: repositories.filter(
                (r) => r.stargazerCount === 0 && r.homepageUrl == null,
              ).length,
              mvp: repositories.filter(
                (r) => r.stargazerCount > 0 && r.stargazerCount < 5,
              ).length,
              production: repositories.filter(
                (r) => r.stargazerCount >= 5 && r.homepageUrl != null,
              ).length,
            ...
            recentActivity: {
              last30Days: {
                commits: Math.round(
                  totalCommits * (recentRepos30 / repositories.length),
                ),
              ...
              last90Days: {
                commits: Math.round(
                  totalCommits * (recentRepos90 / repositories.length),
                ),
            ...
            productivityMetrics: {
              ...
              averageCommitSize: 50, // Estimation basée sur la moyenne GitHub
          ...
          technologyExpertise: {
            overallExpertiseScore: Math.round(
              primaryLanguages.reduce(
                (sum, lang) => sum + lang.proficiencyLevel,
                0,
            ...
            frontendExpertise: {
              ...
              emergingInterests: emergingLanguages
                .filter((l) =>
                  ['Svelte', 'Next.js', 'Nuxt.js'].includes(l.name),
                )
                .map((l) => l.name),
            ...
            advancedLevel: primaryLanguages
              .filter(
                (l) => l.proficiencyLevel >= 60 && l.proficiencyLevel < 80,
              )
              .map((lang) => ({
                technology: lang.name,
                masteryLevel: lang.proficiencyLevel,
                yearsOfExperience: Math.min(
                  activeYears,
                  Math.round(lang.repositoryCount / 4),
                ),
                projectCount: lang.repositoryCount,
            ...
            intermediateLevel: primaryLanguages
              .filter(
                (l) => l.proficiencyLevel >= 40 && l.proficiencyLevel < 60,
              )
              .map((lang) => ({
                technology: lang.name,
                masteryLevel: lang.proficiencyLevel,
                yearsOfExperience: Math.min(
                  activeYears,
                  Math.round(lang.repositoryCount / 5),
                ),
                projectCount: lang.repositoryCount,
          ...
          communityImpact: {
            ...
            maintainedProjects: repositories.filter(
              (r) => !r.isFork && !r.isArchived,
            ).length,
          ...
          growthInsights: {
            growthTrajectory: {
              ...
              sustainability: Math.min(
                100,
                (recentReposYear / repositories.length) * 100,
              ),
            },
            careerProgression: {
            ...
            productivityTrends: [
              {
                ...
                efficiency: Math.min(
                  100,
                  (recentRepos90 / repositories.length) * 100,
                ),
            ...
            technologyEvolution: [],
            skillProgression: [],
            futureGrowthPotential: Math.min(
              100,
          ...
          industryBenchmarks: {
            ...
            categoryPercentiles: [
              ...
              },
              {
                ...
                percentile: Math.min(
                  95,
                  Math.round((userData.followers || 0) * 10 + totalStars * 5),
                ),
            ...
            industryComparisons: [
              {
                ...
                position: primaryLanguages.some((l) =>
                  ['JavaScript', 'TypeScript'].includes(l.name),
                )
                  ? 'Above Average'
                  : 'Average',
                metrics: {
            ...
            competitiveAdvantages: primaryLanguages
              .slice(0, 2)
              .map((l) => `Strong ${l.name} skills`),
            industryTrends: [
              {
                technology: 'TypeScript',
                trend: primaryLanguages.some((l) => l.name === 'TypeScript')
                  ? 'Adopted'
                  : 'Growing',
                adoptionRate: 85,
              ...
              {
                technology: 'AI/ML',
                trend: repositories.some((r) =>
                  r.topics.some((t) => ['ai', 'ml'].includes(t)),
                )
                  ? 'Adopted'
                  : 'Emerging',
                adoptionRate: 45,
