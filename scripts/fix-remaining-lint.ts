...
const CONTROLLER_TYPE_FIXES: Array<[RegExp, string]> = [
  ...
  [
    /logWithContext\.error\([^,]+,\s*([a-zA-Z]+): unknown\)/g,
    'logWithContext.error($1, _$1: unknown)',
  ],
  [/catch \(([a-zA-Z]+): unknown\)/g, 'catch (_$1: unknown)'],

  // Fix unused variable patterns
  [
    /\b(error|analytics|userProfile|index|now|timeframe|roles|next|res|prisma|updatedRepo): /g,
    '_$1: ',
  ],
...
const FILE_SPECIFIC_FIXES: Record<string, Array<[RegExp, string]>> = {
  controllers: [
    ...
    [
      /res\.status\(\d+\)\.json\(([a-zA-Z]+): unknown\)/g,
      'res.status($1).json($1)',
    ],
    [
      /return res\.status\(\d+\)\.json\(\{[^}]+error: unknown[^}]*\}\)/g,
  ...
  services: [
    ...
    [
      /async ([a-zA-Z]+)\([^)]*\): Promise<unknown>/g,
      'async $1(): Promise<Record<string, unknown>>',
    ],
  ],
};
